...
$(document).on('rex:ready', function (event, element) {
    if(!$('body#rex-page-mediapool-tools-bulk-rework').length) {
        return;
    }
    ...
    function startBatchProcessing(filenames) {
        ...
        // Hole aktuelle Einstellungen (versteckt oder nicht)
        // Im neuen AddOn nehmen wir die Werte aus dem Backend, falls sie im DOM verfügbar sind, 
        // oder senden 0 damit der Server die Config nutzt.
        // Da wir im PHP die Werte nicht mehr im DOM anzeigen (aktuell hidden), sollten wir sicherstellen,
        // dass die Logik passt. Wenn 0 gesendet wird, nimmt der Server die Config.
        let maxWidth = 0; 
        let maxHeight = 0;
        
        // Versuche Werte aus dem DOM zu lesen falls vorhanden
        /*
        let maxWidthText = $('.uploader-bulk-rework-current-settings b').first().text();
        let maxHeightText = $('.uploader-bulk-rework-current-settings b').eq(1).text();
        if(maxWidthText) maxWidth = parseInt(maxWidthText);
        if(maxHeightText) maxHeight = parseInt(maxHeightText);
        */

        // Starte Batch
        $.ajax({
            url: window.location.pathname + '?rex-api-call=mediapool_tools_bulk_process',
            method: 'POST',
            dataType: 'json',
            data: {
                action: 'start',
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        if (this.status.status === 'completed') {
                            ...
                            // Schnelleres Polling für parallele Verarbeitung
