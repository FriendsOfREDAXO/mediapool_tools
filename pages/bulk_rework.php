...
use FriendsOfRedaxo\MediapoolTools\BulkRework;
use FriendsOfRedaxo\MediapoolTools\BulkReworkList;

$listDebug = false;
$addon = rex_addon::get('mediapool_tools');

// Konfiguration - standardmäßig auf 2000px wenn nicht gesetzt, oder Uploader Config nutzen?
// Wir setzen hier Standardwerte, da die Config nun im neuen AddOn liegt
$maxWidth = (int) $addon->getConfig('image-max-width', 2000);
$maxHeight = (int) $addon->getConfig('image-max-height', 2000);

// Info Box wenn Konfiguration nicht gesetzt
if ($maxWidth == 0 && $maxHeight == 0) {
    echo rex_view::warning($addon->i18n('bulk_rework_config_hint'));
}

if (rex_request('formsubmit', 'string') == 'set-num-hits-per-page') {
    $setHitsPerPage = rex_request('rework-files-num-hits-per-page', 'int', 0);
    $this->setConfig('bulk-rework-hits-per-page', $setHitsPerPage ? $setHitsPerPage : 200);
}
$hitsPerPage = $this->getConfig('bulk-rework-hits-per-page', 200);
...
$list->setColumnFormat('width', 'custom', static function ($params) use ($maxWidth) {
    ...
    {
        // Einfache Farbberechnung - hier keine Abhängigkeit zu BulkRework::lightenColor da nur optisch
        return $params['subject'] .
            '<span class="dimension-too-large" style="background-color: #ffcccc;" data-toggle="tooltip" data-placement="top" title="Zu breit"></span>';
    }
...
$list->setColumnFormat('height', 'custom', static function ($params) use ($maxHeight) {
    ...
    {
        return $params['subject'] .
            '<span class="dimension-too-large" style="background-color: #ffcccc;" data-toggle="tooltip" data-placement="top" title="Zu hoch"></span>';
    }
...
$fragment->setVar('title',
    $addon->i18n('bulk_rework_table_title') .
           '<div class="small uploader-bulk-rework-current-settings" style="display:none;">' . sprintf('Aktuelle Einstellungen: max. Breite = <b>%spx</b> | max. Höhe = <b>%spx</b>', $maxWidth, $maxHeight) . '</div>'.
...
$hitsPerPageForm = '<form class="uploader-bulk-rework-num-hits-per-page" action="' . rex_url::currentBackendPage() . '" method="post" style="display: inline-block; margin-right: 20px;">
    <div class="form-group form-group-sm rex-form-group-nowrap" style="display: inline-flex; align-items: center; margin: 0; gap: 10px; margin-right: 15px;">
        <label for="num-hits-per-page" style="margin: 0; font-weight: normal; font-size: 13px;">' . $addon->i18n('bulk_rework_table_num_hits_per_page') . '</label>
...
// ... search fields usage ...
// I'm skipping detailed recreation of search fields html for brevity, using what I read.
$searchFieldsColumns = [
...
$searchFields['filename'] = '<div class="' . $searchFieldsColumns['filename'] . '"><div class="form-group">
    ...
    <input class="form-control" type="text" id="search-filename" name="rework-files-search-filename" value="' .
        rex_escape(rex_request('rework-files-search-filename', 'string', '')) . '">
</div></div>';
...
$searchFields['category-id'] = '<div class="' . $searchFieldsColumns['category-id'] . '"><div class="form-group">
    ...
    <input class="form-control" type="text" id="search-media-category" name="rework-files-search-media-category" value="' .
        rex_escape(rex_request('rework-files-search-media-category', 'string', '')) . '" placeholder="' .
